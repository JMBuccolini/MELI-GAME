...
function Card({ title, src, addCard, product,isClickable, index,shipping,link,price }) {
  ...
  //Este endpoint funcionaba en la v.1 pero ahora se necesita token
  ...
  const handleButtonClick = (product, index) => {
    ...
    setGlow(true)
    // Se fija si las cartas hacen match y devuelve un booleano
    const cardsMatch = addCard(product, index);
    
    // Si las cartas no coinciden, se dan vuelta solas
    if (!cardsMatch) {
      setTimeout(()=>{
    ...
    >
      ...
      >
        ...
        <div className="flip-card-back h-[300px] flex flex-col items-center justify-center overflow-hidden relative">
          <img src={thumbnail_id} alt={title} className="w-full rounded-lg object-contain h-full" />
